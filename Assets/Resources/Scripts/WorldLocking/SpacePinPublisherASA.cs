...
namespace Microsoft.MixedReality.WorldLocking.ASA
{
    ...
    /// </remarks>
    public class SpacePinPublisherASA : MonoBehaviour, IPublisher
    {
        ...
        private async Task<AnchorRecord> DownloadRecord(CloudAnchorId cloudAnchorId)
        {
            ...
            // since SpacePinASAInit.cs will try to setup/download the spacepin on Start() and because anchorLocateCriteria gets initialized on Awake(), we may arrive here before
            // anchorLocateCriteria has been initialized and thus may need to wait until it is
            waitForAnchorLocateCriteria = 100;
            bool waiting = anchorLocateCriteria == null;
            if (waiting) 
            {
                Debug.Log("Waiting for anchorLocateCriteria to become active..");
                await Task.Delay(waitForMoreAnchorsTimeoutMS);
                waiting = anchorLocateCriteria == null;
            }
